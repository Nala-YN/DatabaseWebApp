    ...
    <div class="form-wrapper">
      ...
      <div v-if="step === 1" class="input-wrapper">
        <div class="border-wrapper">
          <input type="text" name="username" placeholder="请输入用户昵称" class="border-item" autocomplete="off"
            v-model="username" />
        </div>
        <div class="border-wrapper">
          <input type="email" name="email" placeholder="请输入邮箱" class="border-item" autocomplete="off" v-model="email" />
        </div>
        <div class="border-wrapper">
          <input type="password" name="password" placeholder="请输入密码" class="border-item" autocomplete="off"
            v-model="password" />
        </div>
        <div class="border-wrapper">
          <input type="password" name="confirm" placeholder="确认密码" class="border-item" autocomplete="off"
            v-model="confirmPassword" />
        </div>
      </div>
      <div v-else class="input-wrapper">
        <div class="border-wrapper">
          <input type="text" name="phone" placeholder="电话号码（选填）" class="border-item" autocomplete="off" pattern="[0-9]{10}"
            v-model="phoneNum" />
        </div>
        <div class="border-wrapper">
          <input type="text" name="campus" placeholder="所在校区（选填）" class="border-item" autocomplete="off" v-model="campus" />
        </div>
        <div class="border-wrapper">
          <input type="text" name="address" placeholder="地址（选填）" class="border-item" autocomplete="off"
            v-model="address" />
        </div>
      </div>
...
export default {
  ...
  methods: {
    nextPage() {
      ...
      else if (this.phoneNum.length !== 0 && !phoneReg.test(this.phoneNum)) {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success) {
            ...
            // 已知错误类型：邮箱账户已存在
            ElMessage({ message: "注册失败"+ response.data.message, type: 'error' });
            this.step=1;
          }
        })
        .catch((error) => {
